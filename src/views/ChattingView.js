...
export default class ChattingView extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showEmojiView: false,
    ...
    if (this.state.showMoreView) {
      moreView.push(
        <View key={"more-view-key"}>
          <View style={{width: width, height: 1 / PixelRatio.get(), backgroundColor: HelperColors.dividerColor}}/>
          <View style={{height: Global.emojiViewHeight}}>
            <MoreView
  ...
  renderReceivedTextMsg(item) { // 接收的文本消息
    let contactAvatar = require('../../img/wechat/avatar.png');
  ...
  renderSendTextMsg(item) { // 发送出去的文本消息
    let avatar = require('../../img/wechat/avatar.png');
